...
const FacultyAssignmentAdder = ({ studentData, classDetails, sessionHours, onReset }) => {
  ...
  const position = 'Grader'; // locked to Grader only
  ...
  const calculateComp = (pos, hours, edu, fellow, session) => {
    ...
    if (pos === "Grader" && edu === "BS" && fellow === "No") {
      if (h === 5  && (sess === "A" || sess === "B")) return 781;
      if (h === 5  && sess === "C") return 1562;
      if (h === 10 && (sess === "A" || sess === "B")) return 1562;
      if (h === 10 && sess === "C") return 3124;
  ...
  const acknowledgeText = `I agree to hiring ${studentName} for Grader at $${compensation.toLocaleString()} for ${classLabel}`;
  ...
  return (
    <Paper elevation={3} sx={{ p: 4, mt: 4 }}>
      ...
      {/* FORM FIELDS — Position is locked to Grader */}
      <Grid container spacing={3}>
        <Grid item xs={12} sm={4}>
          <FormControl fullWidth sx={{ minWidth: 250 }}>
            <InputLabel>Position</InputLabel>
            <Select value={position} label="Position" disabled>
